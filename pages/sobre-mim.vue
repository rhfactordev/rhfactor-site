...
// TODO: Implementar em todas as páginas
useSchemaOrg([
  defineOrganization({
    name: site.name,
    logo: site.logo,
    sameAs: site.socialLink
  }),
  definePerson({
  ...
  defineImage({
    url: about.image,
    caption: `Imagem de ${about.personName}`
  })
])
...
<template>
  <main>
    <e-title>{{ about.title }}</e-title>
    <Breadcrumbs class="breadcrumb" />
    <section class="m-auto w-full p-5 md:w-8/12">
      <div class="grid grid-cols-1 md:grid-cols-2 md:gap-10">
        <div class="mb-6">
          <img class="w-full" :src="about.image" :alt="`Imagem de ${about.title}`" />
        </div>
